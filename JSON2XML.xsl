...
<!-- Setting variables for xml and xsl -->
...
<!-- Body of the structure, this template call all the other ones -->
        ...
        <so:output>
            <xsl:choose>
                <xsl:when test="$parent-ele">
					<xsl:if test="$parent-ele='header'">
						<xsl:element name="metadata">
							<xsl:copy-of select="exsl:node-set($t2)/so:output/node()" />
						</xsl:element>
					</xsl:if>
					<xsl:if test="$parent-ele='body'">
						<xsl:element name="customerIdentification">
							<xsl:copy-of select="exsl:node-set($t2)/so:output/node()" />
						</xsl:element>
					</xsl:if>
					<xsl:if test="$parent-ele='customProperties'">
